...
const CelkovaCislaView = ({ isComparison = false, paneRole = 'single', filtersFromParent, onFiltersChange }) => {
    ...
    const [quickKey, setQuickKey] = useState(() => detectQuickRangePreset(
        buildInitialCelkovaFilters().start_date,
        buildInitialCelkovaFilters().end_date,
    ));
            ...
            {data && !loading && (
                <>
                    ...
                    {detailOpen && !isComparison && (
                        <Modal
                            title={`${detailTitle}${selectedEntity?.stredisko ? ` – ${selectedEntity.stredisko}` : ''}`}
                            titleAs="h4"
                            size="lg"
                            onClose={closeDetail}
                        >
                    ...
                    {allegroDetailOpen && !isComparison && (
                        <Modal title="Detail kanálu: Allegro" titleAs="h4" size="lg" onClose={closeAllegroDetail}>
                    ...
                    {eshopDetailOpen && !isComparison && (
                        <Modal title="Detail kanálu: E-shop (bez ALLEGRO)" titleAs="h4" size="lg" onClose={closeEshopDetail}>
                            {renderEshopContent()}
                        </Modal>
                    )}
                    ...
                    {zasilkovnaDetailOpen && !isComparison && (
                        <Modal title="📦 Zásilkovna - Rozpad podle prodejen" titleAs="h4" size="lg" onClose={closeZasilkovnaDetail}>
                            {renderZasilkovnaContent()}
                        </Modal>
                    )}
