...
def finance_status(request):
    ...
    packeta_admin = get_packeta_admin_credentials()
    ...
    return _no_store_response({
        ...
        'packeta': {
            'csv_upload': True,
            'fetch_available': packeta_admin is not None,
            'admin_configured': packeta_admin is not None,
            'admin_label': packeta_admin['label'] if packeta_admin else None,
        },
        'manual_naklady': True,
    })
...
def packeta_import_csv(request):
    ...
    log_finance_audit(
        request, 'packeta_import_csv',
        f'prodejna={prodejna_id} created={imp["created"]} skipped={imp["skipped"]}',
    )
...
def packeta_fetch_all(request):
    ...
    log_finance_audit(
        request, 'packeta_fetch_all',
        f'days={days} branches={len(result["branches"])}',
    )
