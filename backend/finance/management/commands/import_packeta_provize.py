...
from finance.packeta_fetch import (
    default_chunk_days,
    fetch_and_import_all_branches,
    fetch_and_import_branch,
    import_packeta_rows,
)
from finance.packeta_parser import parse_packeta_csv
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--all-branches', action='store_true', help='Všechny pobočky (jen s --period days)')
    ...
    def _handle_fetch(self, options):
        ...
        if period in ('month', 'yesterday'):
            if not fetch_pid:
                raise CommandError(
                    f'--period {period} vyžaduje --prodejna-id (jedna pobočka najednou).'
                )
            if fetch_pid not in range(1, 7):
