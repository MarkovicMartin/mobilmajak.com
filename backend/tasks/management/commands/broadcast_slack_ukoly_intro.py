...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for user in qs:
            ...
            label = f"#{user.id} {user.jmeno} {user.prijmeni} ({user.role}, {user.email or 'bez e-mailu'})"
            if not slack_id:
                skipped += 1
                skipped_users.append(label)
                self.stdout.write(f"  SKIP {label} – bez Slacku")
                continue
