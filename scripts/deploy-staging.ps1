...
Invoke-Ssh "sed -i 's/\r$//' /tmp/staging-post-deploy.sh; bash /tmp/staging-post-deploy.sh || true; rm -f /tmp/staging-post-deploy.sh; chmod -R a+rX ${StagingPath}/frontend/build/static; systemctl reload nginx"
