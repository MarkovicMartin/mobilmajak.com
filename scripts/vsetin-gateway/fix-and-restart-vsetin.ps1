# Vsetin: aktualizace watchdog + config + restart. Spustit jako spravce.
# ASCII only

$ErrorActionPreference = "Stop"
$Task = "Mobilmajak-CameraGateway-Vsetin"
$Dir = "C:\ProgramData\Mobilmajak\CameraGateway-Vsetin"
$SourceDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$GwDir = Join-Path $SourceDir "..\camera-gateway"

if (-not (Test-Path $GwDir)) {
    throw "Missing $GwDir - na USB musi byt camera-gateway i vsetin-gateway"
}
if (-not (Test-Path $Dir)) {
    throw "Gateway neni nainstalovana: $Dir - nejdriv install-vsetin-camera-gateway.cmd"
}

Write-Host "=== Vsetin: fix (camera-gateway) + restart ===" -ForegroundColor Cyan
