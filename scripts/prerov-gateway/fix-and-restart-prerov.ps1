# Prerov: aktualizace watchdog + config + restart. Spustit jako spravce.
# ASCII only

$ErrorActionPreference = "Stop"
$Task = "Mobilmajak-CameraGateway-Prerov"
$Dir = "C:\ProgramData\Mobilmajak\CameraGateway-Prerov"
$SourceDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$GwDir = Join-Path $SourceDir "..\camera-gateway"

if (-not (Test-Path $GwDir)) {
    throw "Missing $GwDir - na USB musi byt camera-gateway i prerov-gateway"
}
if (-not (Test-Path $Dir)) {
    throw "Gateway neni nainstalovana: $Dir - nejdriv install-prerov-camera-gateway.cmd"
}

Write-Host "=== Prerov: fix (camera-gateway) + restart ===" -ForegroundColor Cyan
