# MOBILMAJAK Globus camera gateway installer
# Requires scripts/camera-gateway on the same machine (copy both folders to USB).
# ASCII only

param(
    [switch]$SkipTest,
    [string]$InstallDir = "C:\ProgramData\Mobilmajak\CameraGateway-Globus"
)
...
if (-not (Test-Path $GwInstall)) {
    throw "Missing $GwInstall - copy scripts/camera-gateway next to globus-gateway"
}
