#!/bin/bash
# Rychlý test pilotu Zlin / Cepkov (čte secrets/camera_motion_zlin.json)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CFG="$ROOT/secrets/camera_motion_zlin.json"
PY="$ROOT/backend/.venv/bin/python3"
SCRIPT="$ROOT/scripts/camera_motion_gateway.py"

if [ ! -f "$CFG" ]; then
  echo "Chybí $CFG"
  echo "Zkopírujte: cp scripts/zlin-gateway/config.example.json secrets/camera_motion_zlin.json a doplňte"
  exit 1
fi
