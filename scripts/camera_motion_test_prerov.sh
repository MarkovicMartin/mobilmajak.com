#!/bin/bash
# Rychlý test pilotu Přerov (čte secrets/camera_motion_prerov.json)
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
CFG="$ROOT/secrets/camera_motion_prerov.json"
PY="$ROOT/backend/.venv/bin/python3"
SCRIPT="$ROOT/scripts/camera_motion_gateway.py"

if [ ! -f "$CFG" ]; then
  echo "Chybí $CFG"
  echo "Zkopírujte: cp scripts/prerov-gateway/config.example.json secrets/camera_motion_prerov.json a doplňte"
  exit 1
fi
