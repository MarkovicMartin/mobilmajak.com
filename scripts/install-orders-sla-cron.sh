#!/bin/bash
# Denní SLA připomínky objednávek (Slack) – cron na VPS. Idempotentní.
...
MARKER="# mobilmajak-orders-sla-${LABEL}"
TMP=/tmp/webmajak-crontab-orders-sla-${LABEL}.tmp

sudo -u webmajak crontab -l 2>/dev/null | grep -v "\$MARKER" | grep -v 'check_orders_sla_reminders' > "\$TMP" || true
...
echo "=== webmajak crontab (orders SLA ${LABEL}) ==="
sudo -u webmajak crontab -l | grep -E 'check_orders_sla_reminders|mobilmajak-orders-sla' || true
EOF

echo "Cron pro SLA připomínky objednávek nastaven (${LABEL}, 7:20 denně)."
